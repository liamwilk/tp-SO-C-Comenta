...
void planificacion_largo_plazo(hilos_args *hiloArgs, t_diagrama_estados *estados, t_log *logger)
{
    ...
    if (cant_en_ready < hiloArgs->kernel->gradoMultiprogramacion)
    {
        while (cant_en_ready < (hiloArgs->kernel->gradoMultiprogramacion) && cant_en_new > 0)
        {
            ...
            {
                kernel_log_generic(hiloArgs, LOG_LEVEL_WARNING, "No se puede mover el proceso PID: <%d> a ready, ya que no fue aceptado por memoria", proceso->pid);
                continue;
            }
            proceso_push_ready(estados, proceso, logger);
            cant_en_ready++;
            cant_en_new--;
            kernel_log_generic(hiloArgs, LOG_LEVEL_DEBUG, "[PLANIFICADOR LARGO PLAZO] Proceso PID: <%d> movido a ready", proceso->pid);
        }
    }
    if (cant_en_new != 0)
    {
...
void fifo(hilos_args *hiloArgs, t_diagrama_estados *estados, t_log *logger)
{
    ...
    {
        kernel_log_generic(hiloArgs, LOG_LEVEL_DEBUG, "[FIFO]: No hay procesos en ready");
        return;
    }

    kernel_log_generic(hiloArgs, LOG_LEVEL_DEBUG, "[FIFO]: Procesos en ready: %d", list_size(estados->ready));
    kernel_log_generic(hiloArgs, LOG_LEVEL_DEBUG, "[FIFO]: Procesos en exec: %d", list_size(estados->exec));

    if (list_size(estados->ready) > 0 && list_size(estados->exec) == 0)
    {
        kernel_log_generic(hiloArgs, LOG_LEVEL_DEBUG, "[FIFO] Entré al segundo IF");

        t_pcb *aux = kernel_transicion_ready_exec(estados, hiloArgs->kernel, hiloArgs->logger);
        kernel_log_generic(hiloArgs, LOG_LEVEL_DEBUG, "[FIFO]: Procesos en ready LUEGO de transicionar de READY a EXEC: %d", list_size(estados->ready));
        kernel_log_generic(hiloArgs, LOG_LEVEL_DEBUG, "[FIFO]: Enviando proceso <PID: %d> a CPU", aux->pid);
        free(aux);
    }
}
...
void round_robin(hilos_args *hiloArgs, t_diagrama_estados *estados, t_log *logger)
{
    if (list_size(estados->ready) == 0)
    {
        kernel_log_generic(hiloArgs, LOG_LEVEL_DEBUG, "[ROUND ROBIN]: No hay procesos en ready");
        return;
    }
    if (list_size(estados->exec) == 0)
    {
        ...
        t_pcb *aux = kernel_transicion_ready_exec(estados, hiloArgs->kernel, hiloArgs->logger);
        kernel_log_generic(hiloArgs, LOG_LEVEL_DEBUG, "[ROUND ROBIN]: Enviando proceso <PID: %d> a CPU", aux->pid);
