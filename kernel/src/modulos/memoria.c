...
void switch_case_memoria(t_log *logger, t_op_code codigo_operacion, hilos_args *args, t_buffer *buffer)
{
    ...
    case MEMORIA_KERNEL_NUEVO_PROCESO:
    {
        ...
        imprimir_log(args, LOG_LEVEL_DEBUG, "Recibí la respuesta de Memoria acerca de la solicitud de nuevo proceso");
        imprimir_log(args, LOG_LEVEL_DEBUG, "PID: %d", proceso->pid);
        imprimir_log(args, LOG_LEVEL_DEBUG, "Cantidad de instrucciones: %d", proceso->cantidad_instrucciones);
        imprimir_log(args, LOG_LEVEL_DEBUG, "Leido: %d", proceso->leido);
        if (proceso->leido)
        {
            ...
            imprimir_log(args, LOG_LEVEL_DEBUG, "Proceso PID:<%d> aceptado en memoria", pcb->pid);
            imprimir_log(args, LOG_LEVEL_INFO, "Proceso PID:<%d> aceptado en memoria", pcb->pid);
        }
        else
        {
            imprimir_log(args, LOG_LEVEL_ERROR, "Proceso PID:<%d> rechazado en memoria", proceso->pid);
            // Eliminar proceso de la cola de new
            proceso_revertir(args->estados, proceso->pid);
            imprimir_log(args, LOG_LEVEL_WARNING, "Proceso PID:<%d> eliminado de kernel", proceso->pid);
        };
        free(proceso);
        break;
    }
