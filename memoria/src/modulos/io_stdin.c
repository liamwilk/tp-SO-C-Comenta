...
void switch_case_entrada_salida_stdin(t_args_hilo *argumentos, t_op_code codigo_operacion, t_buffer *buffer)
{
	switch (codigo_operacion)
	{
		case MEMORIA_ENTRADA_SALIDA_IDENTIFICACION:
        {
            ...
            log_debug(argumentos->argumentos->logger, "[IO STDIN/Interfaz %s/Orden %d] Se recibio identificador.",identificacion->identificador, argumentos->entrada_salida->orden);

            break;
        }
		default:
		{
			log_warning(argumentos->argumentos->logger, "[STDIN] Se recibio un codigo de operacion desconocido. Cierro hilo");
